...
document.querySelector("#btn1").addEventListener("click",()=>{
  ...
  // ul.chlidNodes[9]의 다음 다음 형제 노드 선택
  // == ul.chlidNodes[11];
  console.log(ul.childNodes[9].nextSibling.nextSibling) // 11번
  
  // ul의 마지막 자식 노드의 이전 형제 노드 선택
  console.log(ul.childNodes[12].previousSibling) // 11번

  // 노드 추가

  // 1.마지막 자식 노드로 추가 (요소.appendChild(노드))
  //  - > list[11]의 마지막 자식으로 "ㅋㅋㅋ"라는 TestNode추가
  list[11].appendChild(document.createTextNode("ㅋㅋㅋ"));



  // 2.마지박 자식으로 추기(요소.append("내용"or 노드))
  list[11].append("123","abc","가나다라");
  ...
  // 4.이전/다음 형제로 추가 (요소.before("내용"or노드))/요소.after("내용or노드")
  // ul의 전/후에 내용 추가
  ul.before("이전입니다","ㅎㅎㅎ");
  ul.after("이후입니다");
